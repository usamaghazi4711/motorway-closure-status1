...
const App = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        // Load plaza statuses
        const statusResult = await window.storage.get('motorway-plaza-statuses');
        const diversionResult = await window.storage.get('motorway-diversions');
        const updateResult = await window.storage.get('motorway-last-updated');
        
        if (statusResult && statusResult.value) {
          setPlazaStatuses(JSON.parse(statusResult.value));
        } else {
          // Initialize default data if nothing saved
          const initial = {};
          ...
          await window.storage.set('motorway-plaza-statuses', JSON.stringify(initial));
        }
        
        if (diversionResult && diversionResult.value) {
          setDiversions(JSON.parse(diversionResult.value));
        } else {
          ...
          await window.storage.set('motorway-diversions', JSON.stringify(initialDiversions));
        }
        
        if (updateResult && updateResult.value) {
          setLastUpdated(updateResult.value);
        } else {
          const now = new Date().toISOString();
          setLastUpdated(now);
          await window.storage.set('motorway-last-updated', now);
        }
      } catch (error) {
  ...
  const toggleDiversion = async (motorway, fromPlaza, toPlaza, direction) => {
    ...
    const newDiversions = {
      ...
      [motorway]: {
        ...diversions[motorway],
        [key]: {
          ...diversions[motorway]?.[key],
          [direction]: !diversions[motorway]?.[key]?.[direction]
        }
      }
    };
    ...
    try {
      await window.storage.set('motorway-diversions', JSON.stringify(newDiversions));
    } catch (error) {
  ...
  const saveStatuses = async () => {
    ...
    try {
      await window.storage.set('motorway-plaza-statuses', JSON.stringify(plazaStatuses));
      await window.storage.set('motorway-diversions', JSON.stringify(diversions));
      await window.storage.set('motorway-last-updated', now);
